#!/usr/bin/env python3
"""
Execute a fast heuristic-only trading cycle (no OpenRouter/LLM calls).
Uses deterministic logic for regime detection and strategy selection.
"""
import asyncio
import sys
...
async def execute_heuristic_cycle():
    """Execute a trading cycle using heuristic mode only (no LLM)."""
    
    print("\n" + "="*80)
    print("HEURISTIC-ONLY TRADING CYCLE - NO LLM")
    print("="*80)
    ...
    print(f"AI Mode: HEURISTIC ONLY (No OpenRouter/LLM)")
    print("="*80)
    
    trading_service = None
    
    try:
        # Initialize trading service with OpenRouter DISABLED
        print("\n🔧 Initializing LiveTradingService (heuristic mode)...")
        trading_service = LiveTradingService(
            exchange_name="bybit",
            use_testnet=False,  # Bybit Demo uses demo domain
            use_openrouter=False  # CRITICAL: Disable LLM calls
        )
        
        symbol = "XAUUSDT"
        user_id = "test_user"
        
        print(f"\n🚀 Starting heuristic trading cycle for {symbol}...")
        print(f"   This will use deterministic logic instead of LLM:")
        print(f"   • Regime detection: Heuristic (volatility × trend)")
        print(f"   • Strategy selection: Rule-based mapping")
        print(f"   • Risk assessment: Deterministic calculations")
        print(f"   • Expected time: < 5 seconds (vs 30-60s with LLM)")
        ...
        print("\n" + "="*80)
        if status == 'success':
            print("✅ HEURISTIC CYCLE COMPLETED SUCCESSFULLY!")
            print("="*80)
            print("\nThe system successfully executed a complete trading cycle")
            print("using deterministic heuristic logic (no LLM calls).")
...
def main():
    """Main entry point."""
    success = asyncio.run(execute_heuristic_cycle())
