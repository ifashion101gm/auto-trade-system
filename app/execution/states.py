...
# =============================================================================
# Order Lifecycle State Machine (Step 9)
# =============================================================================


class OrderState(Enum):
    """
    States for individual order lifecycle management.
    
    State transitions enforce strict ordering to prevent invalid state changes.
    Terminal states (FILLED, CANCELED, REJECTED, EXPIRED) cannot transition further.
    """
    
    NEW = "new"                      # Created locally, not sent
    PENDING = "pending"              # Sent to exchange, awaiting confirmation
    PARTIALLY_FILLED = "partially_filled"  # Partial execution received
    FILLED = "filled"                # Fully executed
    CANCELED = "canceled"            # Explicitly canceled
    REJECTED = "rejected"            # Rejected by exchange
    EXPIRED = "expired"              # Time-out or validity ended
    RECOVERY_REQUIRED = "recovery_required"  # State mismatch detected
    
    def __str__(self):
        return self.value


# Valid order state transitions (state -> allowed next states)
...
def is_valid_order_state_transition(from_state: OrderState, to_state: OrderState) -> bool:
    """
    ...
    Returns:
        True if transition is allowed
    """
    allowed_transitions = ORDER_STATE_TRANSITIONS.get(from_state, [])
    return to_state in allowed_transitions


def get_valid_next_order_states(current_state: OrderState) -> list:
    """
    Get list of valid next order states from current state.
